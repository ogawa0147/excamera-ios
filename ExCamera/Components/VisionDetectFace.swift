...
// from https://github.com/mattlawer/FaceLandmarksDetection/blob/master/DetectFaceLandmarks/FaceLandmarksDetector.swift
final class VisionDetectFace: VisionDetector {
    private let ciContext: CIContext = CIContext()

    func highlightFaces(for source: UIImage, complete: @escaping (UIImage) -> Void) {
        ...
        let vnImage = VNImageRequestHandler(ciImage: source.ciImage!, options: [:])
    ...
    private func drawOnImage(source: UIImage, boundingRect: CGRect, faceLandmarks: VNFaceLandmarks2D) -> UIImage {
        ...
        // draw image
        let cgImage = ciContext.createCGImage(source.ciImage!, from: source.ciImage!.extent)
        ...
        func drawFeature(_ feature: VNFaceLandmarkRegion2D, color: CGColor, close: Bool = false) {
            ...
            for point in feature.normalizedPoints {
                ...
                context.translateBy(x: 0.0, y: source.size.height)
                context.scaleBy(x: 1.0, y: -1.0)
                let map = CGPoint(x: boundingRect.origin.x * source.size.width + point.x * rectWidth, y: source.size.height - (boundingRect.origin.y * source.size.height + point.y * rectHeight))
                context.fillEllipse(in: CGRect(origin: CGPoint(x: map.x-2.0, y: map.y-2), size: CGSize(width: 4.0, height: 4.0)))
                if let index = feature.normalizedPoints.index(of: point) {
                    NSString(format: "%d", index).draw(at: map, withAttributes: textFontAttributes)
                }
